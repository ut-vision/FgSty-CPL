...
def stylization(stylization_module, smoothing_module, content_image_path, style_image_path, content_seg_path, style_seg_path, output_image_path,
                cuda, gpu_id=0, save_intermediate=None, no_post=None, cont_seg_remapping=None, styl_seg_remapping=None):
    # Load image
    with torch.no_grad():
        ...
        try:
            cont_seg = Image.open(content_seg_path).convert('L')
            styl_seg = Image.open(style_seg_path).convert('L')
            cont_seg.resize((new_cw,new_ch),Image.NEAREST)
            styl_seg.resize((new_sw,new_sh),Image.NEAREST)
        except:
            print("fail to load masks")
            cont_seg = []
            styl_seg = []
        ...
        if cuda:
            cont_img = cont_img.cuda(gpu_id)
            styl_img = styl_img.cuda(gpu_id)
            stylization_module.cuda(gpu_id)
        ...
        if save_intermediate:
            ...
            utils.save_image(stylized_img.data.cpu().float(), output_image_path.split(".")[0]+"_sty.png", nrow=1, padding=0)

            with Timer("Elapsed time in propagation: %f"):
                out_img = smoothing_module.process(output_image_path.split(".")[0]+"_sty.png", content_image_path)
            out_img.save(output_image_path.split(".")[0]+"_prop.png")
            ...
            if no_post is False:
                with Timer("Elapsed time in post processing: %f"):
                    out_img = smooth_filter(output_image_path.split(".")[0]+"_prop.png", content_image_path, f_radius=15, f_edge=1e-1, gpu_id=gpu_id)
            out_img.save(output_image_path.split(".")[0]+"_post.png")
        else:
            with Timer("Elapsed time in stylization: %f"):
                stylized_img = stylization_module.transform(cont_img, styl_img, cont_seg, styl_seg)
            if ch != new_ch or cw != new_cw:
            ...
            if no_post is False:
                with Timer("Elapsed time in post processing: %f"):
                    out_img = smooth_filter(out_img, cont_pilimg, f_radius=15, f_edge=1e-1, gpu_id=gpu_id)
